...
class Subject:
    def __init__(self, num_of_chunks=7, decay_factor=3):
        ...
        # self.WM_response = []
        self.WM = [0 for i in range (self.max_size_WM)]   # working memory with num_of_chunk chunks
        self.emotion_scores = [0 for i in range (self.max_size_WM)]   # to keep a track of decaying emotional effect from words seen prior
        ...
        self.lexicon = read_data(self.filename)   # reading the Micro-WNOp file
    ...
    def get_response_time(self, word):
        ...
        # time.sleep(res_time)    # pause program execution to simulate the delay in response according to valence of current (and previous) word(s)
        return res_time
